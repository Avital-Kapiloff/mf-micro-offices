 ...
 ********************************************************************************/
	function mf_process_form($dbh,$input){
		...
		}else if(!empty($logic_field_enable) && $is_edit_page === true){
			//if this edit entry page and has logic enabled, disable all "required" fields
	...
	function mf_get_merchant_redirect_url($dbh,$form_id,$entry_id){
		...
		if(!empty($row['form_redirect_enable']) && !empty($row['form_redirect'])){
			$form_redirect  = mf_parse_template_variables($dbh,$form_id,$entry_id,$row['form_redirect']);
		}
