 ...
 ********************************************************************************/
	...
	//from email address
	$custom_from_email_list = array();
	$custom_from_email_list[0]['label'] = "no-reply@{$domain}";
	$custom_from_email_list[0]['value'] = "no-reply@{$domain}";
	$custom_from_email_list = array_merge($custom_from_email_list,$email_fields);
		
	$array_max_index = count($custom_from_email_list);

	$custom_from_email_list[$array_max_index]['label'] = '&#8674; Set Custom Address';
	$custom_from_email_list[$array_max_index]['value'] = 'custom';

	$custom_from_email_list_values = array();
	foreach ($custom_from_email_list as $value) {
		$custom_from_email_list_values[] = $value['value'];
	}
									...
									<ul id="ls_email_rules_group">
										<?php
											foreach ($logic_emails_array as $rule_id => $value) {
												...
												$custom_from_email_custom = '';
												if(!empty($custom_from_email_list_values)){
													if(!in_array($rule_properties->custom_from_email, $custom_from_email_list_values)){
														$custom_from_email_custom = $rule_properties->custom_from_email;
														$rule_properties->custom_from_email = 'custom';
													}
												}
										?>
																	...
																	<td>
																		...
																		<div class="ls_email_rules_custom_template" id="ls_email_custom_template_div_<?php echo $rule_id; ?>" <?php if($rule_properties->template_name != 'custom'){ echo 'style="display: none"'; } ?>>
																			...
																			<div class="ls_email_rules_custom_template_body">
																				...
																				<label class="description" for="customfromemail_<?php echo $rule_id; ?>">From Email Address <img class="helpmsg" src="images/icons/68_green_whitebg.png" style="vertical-align: top" title="If your form has 'Email' field type, it will be available here and you can choose it as the sender address. Or you can set your own 'From Email Address'"/></label>
																				<select name="customfromemail_<?php echo $rule_id; ?>" id="customfromemail_<?php echo $rule_id; ?>" class="element select medium custom_from_email_dropdown"> 
																					<?php
																						foreach ($custom_from_email_list as $data){
																							if($rule_properties->custom_from_email == $data['value']){
																								$selected = 'selected="selected"';
																							}else{
																								$selected = '';
																							}
																				...
																				<span id="customfromemailspan_<?php echo $rule_id; ?>" <?php if(empty($custom_from_email_custom)){ echo 'style="display: none"'; } ?>>&#8674; <input id="customfromemailuser_<?php echo $rule_id; ?>" name="customfromemailuser_<?php echo $rule_id; ?>" class="element text custom_from_email_text" style="width: 44%" value="<?php echo $custom_from_email_custom; ?>" type="text"></span>
