...
$(function(){
    ...
    //delegate change event to the 'custom from email' dropdown
    $('#ls_box_email_rules').delegate('select.custom_from_email_dropdown', 'change', function(e) {
		var temp = $(this).attr("id").split("_");
		var from_email = $(this).val();
		
		if(from_email == 'custom'){
			$("#customfromemailspan_" + temp[1]).show();
			from_email = $("#customfromemailuser_" + temp[1]).val();
		}else{
			$("#customfromemailspan_" + temp[1]).hide();
		}

		$("#liemailrule_" + temp[1]).data('rule_properties').custom_from_email = from_email;
		
    });

    //delegate event to the 'custom from email' text
    $('#ls_box_email_rules').delegate('input.custom_from_email_text', 'keyup mouseout change', function(e) {
    	var temp = $(this).attr("id").split("_");
